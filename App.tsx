...
const App: React.FC = () => {
    ...
    const isApiMode = useMemo(() => state.settings.aiServiceMode === 'GEMINI_API' && !!(typeof process !== 'undefined' && process.env.GEMINI_API_KEY), [state.settings.aiServiceMode]);

    useEffect(() => {
        const loadedState = loadGameState();
        if (loadedState) {
            dispatch({ type: 'SET_HAS_SAVED_GAME', payload: true });
            // Handle aiServiceMode migration if saved game used Gemini but key is now missing
            if (loadedState.settings.aiServiceMode === 'GEMINI_API' && !(typeof process !== 'undefined' && process.env.GEMINI_API_KEY)) {
                loadedState.settings.aiServiceMode = 'LOCAL';
                setNotification("Gemini API key is no longer available. Switched to Local model.");
    ...
    const handleStartGame = useCallback(async (worldInfo: WorldInfoEntry[], worldSummary: string | null, characterInput: CharacterInput, initialPrompt: string, settings: Settings) => {
        ...
        try {
            ...
            if(settings.aiServiceMode === 'GEMINI_API' && !!(typeof process !== 'undefined' && process.env.GEMINI_API_KEY)) {
    ...
    const loadGameFromState = useCallback((savedState: SavedGameState | null) => {
        ...
        savedTurnIndex.current = null;
        if (savedState.settings.aiServiceMode === 'GEMINI_API' && !(typeof process !== 'undefined' && process.env.GEMINI_API_KEY)) {
