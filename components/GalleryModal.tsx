...
const GalleryModal: React.FC<GalleryModalProps> = ({ isOpen, onClose, images }) => {
  ...
  return (
    <div 
      className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center animate-fade-in p-4" 
      aria-modal="true" 
      role="dialog"
      onClick={onClose}
    >
      <div 
        className="bg-slate-800 w-full max-w-6xl h-[90vh] rounded-lg shadow-2xl border border-slate-700 flex flex-col p-6"
          ...
          {images.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {images.map((image, index) => (
                <div
                  key={index}
                  className="aspect-square bg-slate-900 rounded-md overflow-hidden cursor-pointer transition-transform transform hover:scale-105"
