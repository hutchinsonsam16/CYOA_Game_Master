...
export const structureWorldDataWithAI = async (text: string): Promise<WorldInfoEntry[]> => {
    ...
    const response = await llmService.apiCall(ai => ai.models.gemini25Flash().generateContent({
        ...
        generationConfig: {
            ...
            responseSchema: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { key: { type: Type.STRING }, content: { type: Type.STRING } }, required: ["key", "content"] } }
        },
    }));
...
export const generateCharacterDetails = async (characterInput: CharacterInput): Promise<Partial<CharacterInput>> => {
    ...
    const response = await llmService.apiCall(ai => ai.models.gemini25Flash().generateContent({
        ...
        generationConfig: {
            responseMimeType: "application/json",
            responseSchema: { type: Type.OBJECT, properties: { characterClass: { type: Type.STRING }, alignment: { type: Type.STRING }, backstory: { type: Type.STRING }, skills: { type: Type.STRING } }, required: ["characterClass", "alignment", "backstory", "skills"] }
        },
    }));
    if (!response) return {};
    try {
...
export const generateCharacterFlavor = async (character: Omit<Character, 'portraits'>): Promise<{ class: string, quirk: string }> => {
    ...
    const response = await llmService.apiCall(ai => ai.models.gemini25Flash().generateContent({
        ...
        generationConfig: {
            responseMimeType: "application/json",
            responseSchema: { type: Type.OBJECT, properties: { className: { type: Type.STRING }, quirk: { type: Type.STRING } }, required: ["className", "quirk"] }
        },
    }));
